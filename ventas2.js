...
import { formatearMoneda, registrarVenta, registrarVentaConMerma, esperarAuthListo } from "./js/operaciones.js";
import {
...
function mermaDescripcionItem(item) {
	return mermaVendidaDescripcionItem(item) || mermaSinVenderDescripcionItem(item);
}
...
function actualizarResumen() {
	// Calcular totales y ganancias incluyendo merma vendida y merma sin vender
	let ventaTotal = 0;
	let ventaProfit = 0;
...
function renderCarrito() {
	...
	carritoWrap.innerHTML = items.map((item) => {
		...
		const utilidadMermaPrincipal = tipoMermaPrincipal === "vendida"
			...
			: tipoMermaPrincipal === "sin_vender"
				...
				: 0;
		const perdidaMermaSinVender = costoProducto * mermaSinVender;
		const utilidadTotal = utilidadVenta + utilidadMermaPrincipal - perdidaMermaSinVender;

		const lineaResumen = utilidadTotal >= 0 ? `Ganancia estimada ${formatearMoneda(utilidadTotal)}` : `Pérdida estimada ${formatearMoneda(Math.abs(utilidadTotal))}`;
		...
		const mermaPrincipalResumen = tipoMermaPrincipal === "vendida"
			...
			: tipoMermaPrincipal === "sin_vender"
				...
				: "";
		const mermaPrincipalBalance = tipoMermaPrincipal === "vendida"
			? (mermaVendidaPrecioItem(item) - costoProducto) * mermaPrincipalCantidad
			: tipoMermaPrincipal === "sin_vender"
				? -costoProducto * mermaPrincipalCantidad
				: 0;
		...
		return `
			<div class="carrito-item">
				...
				<div class="carrito-meta">Precio unitario2: ${formatearMoneda(precioItem(item))}</div>
	...
	carritoWrap.querySelectorAll("[data-merma-toggle-id]").forEach((input) => {
		input.addEventListener("change", () => {
			actualizarMermaActiva(input.getAttribute("data-merma-toggle-id"), input.checked);
		});
	});
