...
HTML_TEMPLATE = r"""<!doctype html><html lang="es"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>AlphaGraph · __TITLE__</title><script src="https://cdnjs.cloudflare.com/ajax/libs/d3/7.8.5/d3.min.js"></script><style>
:root{--bg:#070a12;--panel:#0d1321ed;--border:#ffffff14;--text:#edf4ff;--muted:#8d9ab0;--cyan:#38bdf8;--green:#34d399;--amber:#fbbf24;--red:#fb7185;--violet:#a78bfa}*{box-sizing:border-box}html,body{height:100%;margin:0;overflow:hidden;background:var(--bg);color:var(--text);font-family:Inter,ui-sans-serif,system-ui,-apple-system,"Segoe UI",sans-serif}body:before{content:"";position:fixed;inset:0;background:radial-gradient(circle at 18% 12%,#123a6255,transparent 33%),radial-gradient(circle at 82% 78%,#12453344,transparent 30%);pointer-events:none}.app{height:100%;display:grid;grid-template:76px 1fr/300px 1fr 350px}header{grid-column:1/-1;display:flex;align-items:center;gap:18px;padding:13px 20px;background:#080c16e8;border-bottom:1px solid var(--border);backdrop-filter:blur(18px);z-index:5}.brand{min-width:265px;display:flex;align-items:center;gap:12px}.logo{width:43px;height:43px;border-radius:14px;display:grid;place-items:center;background:linear-gradient(135deg,var(--cyan),var(--green));color:#041019;font-weight:950;box-shadow:0 8px 30px #38bdf844}.brand h1{font-size:16px;margin:0}.brand p{font-size:11px;color:var(--muted);margin:3px 0}.stats{display:flex;gap:8px;flex:1}.stat{min-width:85px;padding:8px 10px;border:1px solid var(--border);border-radius:11px;background:#ffffff07}.stat b{display:block;font-size:16px}.stat span,.label{font-size:9px;color:var(--muted);text-transform:uppercase;letter-spacing:.11em}.stamp{text-align:right;font:10px ui-monospace,Consolas;color:var(--muted)}.stamp b{display:block;color:var(--green);margin-bottom:4px}aside{background:var(--panel);backdrop-filter:blur(18px);overflow:auto;z-index:4;padding:18px}.left{border-right:1px solid var(--border)}.right{border-left:1px solid var(--border)}.section{margin-bottom:21px}.section .label{display:block;margin:0 0 10px;font-weight:800}input,select,button{width:100%;border:1px solid #2a3851;background:#111827;color:var(--text);border-radius:10px;padding:10px;font:12px inherit;outline:none}input:focus,select:focus{border-color:var(--cyan);box-shadow:0 0 0 3px #38bdf819}.row{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:8px}button{cursor:pointer;font-weight:750}button:hover{border-color:#52709d}.primary{border:0;background:linear-gradient(135deg,#0ea5e9,#10b981);color:#041019}.health{display:grid;grid-template-columns:1fr 1fr;gap:8px}.health div,.metric{padding:10px;border:1px solid #ffffff0d;background:#ffffff05;border-radius:10px}.health b,.metric b{display:block;font-size:18px}.health span,.metric span{font-size:9px;color:var(--muted);text-transform:uppercase}.legend{display:grid;gap:8px}.legend div{display:flex;align-items:center;gap:9px;font-size:11px;color:#c4cede}.dot{width:9px;height:9px;border-radius:50%;box-shadow:0 0 11px currentColor}.bar{width:21px;height:2px;border-radius:3px}main{position:relative;overflow:hidden}.grid{position:absolute;inset:0;background-image:linear-gradient(#ffffff07 1px,transparent 1px),linear-gradient(90deg,#ffffff07 1px,transparent 1px);background-size:36px 36px;mask-image:radial-gradient(circle,#000,transparent 88%)}#graph{position:absolute;inset:0;width:100%;height:100%}.link{stroke:#52617a;stroke-width:1.15;stroke-opacity:.42}.link.calls_imported_symbol{stroke-dasharray:3 3}.link.cycle{stroke:var(--red);stroke-width:2.5}.node{cursor:pointer}.node circle{stroke:#07101c;stroke-width:2}.node circle.risky{stroke:var(--amber);stroke-width:3;filter:drop-shadow(0 0 7px #fbbf2466)}.node circle.error{stroke:var(--red);stroke-width:3}.node text{fill:#dce7f7;font-size:9px;font-weight:650;paint-order:stroke;stroke:#070a12;stroke-width:3px;pointer-events:none}.dim{opacity:.07!important}.hidden{display:none!important}.hint{position:absolute;bottom:17px;left:50%;transform:translateX(-50%);padding:8px 13px;background:#111827dd;border:1px solid var(--border);border-radius:99px;color:var(--muted);font-size:10px}.empty{padding:18px 3px;color:var(--muted);font-size:12px;line-height:1.65}.path{font:11px ui-monospace,Consolas;color:var(--cyan);word-break:break-all}.title{font-size:19px;margin:8px 0}.pills{display:flex;gap:6px;flex-wrap:wrap;margin:10px 0 15px}.pill{font-size:10px;padding:4px 7px;border:1px solid var(--border);background:#ffffff08;border-radius:99px}.purpose{font-size:12px;color:var(--muted);line-height:1.6}.metrics{display:grid;grid-template-columns:1fr 1fr;gap:8px}.list{display:grid;gap:6px}.item{padding:8px 9px;border-left:2px solid var(--cyan);background:#ffffff05;border-radius:0 7px 7px 0;font-size:11px;line-height:1.4;word-break:break-word}.item.warn{border-color:var(--amber)}.item.error{border-color:var(--red)}.tooltip{position:fixed;z-index:20;pointer-events:none;opacity:0;max-width:330px;padding:10px 12px;border:1px solid var(--border);background:#080d18f2;backdrop-filter:blur(14px);border-radius:9px;box-shadow:0 18px 50px #0009;font-size:11px}.tooltip b{color:var(--cyan)}
@media(max-width:1050px){.app{grid-template-columns:260px 1fr}.right{position:fixed;right:0;top:76px;bottom:0;width:330px;box-shadow:-20px 0 50px #0008}.stat:nth-child(n+4){display:none}}@media(max-width:720px){.app{display:block}.left,.right,.stats{display:none}header{height:76px}.stamp{margin-left:auto}main{height:calc(100% - 76px)}}
</style></head><body><div class="app"><header><div class="brand"><div class="logo">A³</div><div><h1>AlphaGraph</h1><p>Mapa arquitectónico · __TITLE__</p></div></div><div class="stats" id="stats"></div><div class="stamp"><b>MAPA VERIFICADO</b><span id="fingerprint"></span></div></header>
<aside class="left"><div class="section"><span class="label">Explorar arquitectura</span><input id="q" placeholder="Buscar archivo o símbolo…"><div class="row"><select id="folder"><option value="">Todas las áreas</option></select><select id="relation"><option value="">Toda relación</option></select></div><div class="row"><button id="reset">Limpiar</button><button class="primary" id="fit">Centrar</button></div></div><div class="section"><span class="label">Salud del proyecto</span><div class="health"><div><b id="errors" style="color:var(--red)">0</b><span>Errores</span></div><div><b id="warnings" style="color:var(--amber)">0</b><span>Advertencias</span></div></div></div><div class="section"><span class="label">Relaciones</span><div class="legend" id="relLegend"></div></div><div class="section"><span class="label">Áreas</span><div class="legend" id="folderLegend"></div></div></aside>
<main><div class="grid"></div><svg id="graph"></svg><div class="hint">Clic: inspeccionar · rueda: zoom · arrastrar: mover · doble clic: limpiar</div></main><aside class="right"><div id="empty" class="empty">Selecciona un archivo para conocer su propósito, símbolos, colecciones, dependencias, consumidores, radio de impacto y diagnósticos.</div><div id="details" class="hidden"></div></aside></div><div class="tooltip" id="tip"></div>
<script>
const data=__DATA_JSON__,stats=data.stats,impact=data.impact||{};const q=document.querySelector('#q'),folder=document.querySelector('#folder'),relation=document.querySelector('#relation'),errors=document.querySelector('#errors'),warnings=document.querySelector('#warnings'),empty=document.querySelector('#empty'),reset=document.querySelector('#reset'),fit=document.querySelector('#fit');const relColors={imports:'#38bdf8',calls_imported_symbol:'#a78bfa',loads_script:'#34d399',loads_stylesheet:'#22d3ee',loads_asset:'#64748b',navigates_to:'#fbbf24',submits_to:'#fb923c'},relNames={imports:'Importa',calls_imported_symbol:'Llama símbolo',loads_script:'Carga script',loads_stylesheet:'Carga CSS',loads_asset:'Usa asset',navigates_to:'Navega',submits_to:'Envía formulario'};
const esc=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c])),edgeId=x=>typeof x==='object'?x.id:x,w=()=>document.querySelector('main').clientWidth,h=()=>document.querySelector('main').clientHeight;
document.querySelector('#fingerprint').textContent=data.fingerprint.slice(0,16)+'… · '+data.generatedAt.slice(0,10);errors.textContent=stats.errors;warnings.textContent=stats.warnings;const statsEl=document.querySelector('#stats');statsEl.innerHTML=[['Archivos',stats.total_files],['Relaciones',stats.total_edges],['Símbolos',stats.total_symbols],['Colecciones',stats.total_firestore_collections],['Ciclos',stats.total_cycles]].map(x=>`<div class="stat"><b>${x[1]}</b><span>${x[0]}</span></div>`).join('');
const svg=d3.select('#graph'),g=svg.append('g'),zoom=d3.zoom().scaleExtent([.08,7]).on('zoom',e=>g.attr('transform',e.transform));svg.call(zoom);const folders=[...new Set(data.nodes.map(d=>d.folder))].sort(),palette=['#38bdf8','#34d399','#a78bfa','#fb7185','#fbbf24','#22d3ee','#fb923c','#818cf8','#2dd4bf','#e879f9'],color=d3.scaleOrdinal(folders,palette),types=[...new Set(data.edges.map(e=>e.type))].sort();folder.innerHTML+=folders.map(x=>`<option>${x}</option>`).join('');relation.innerHTML+=types.map(x=>`<option value="${x}">${relNames[x]||x}</option>`).join('');relLegend.innerHTML=types.map(x=>`<div><i class="bar" style="background:${relColors[x]||'#64748b'}"></i>${relNames[x]||x}</div>`).join('');folderLegend.innerHTML=folders.slice(0,12).map(x=>`<div><i class="dot" style="background:${color(x)};color:${color(x)}"></i>${x}</div>`).join('');
const maxD=d3.max(data.nodes,d=>d.degree)||1,maxR=d3.max(data.nodes,d=>d.risk_score)||1,radius=d=>d.kind==='asset'?3.5:5+11*Math.sqrt((d.degree||0)/maxD),sim=d3.forceSimulation(data.nodes).force('link',d3.forceLink(data.edges).id(d=>d.id).distance(d=>d.type==='calls_imported_symbol'?95:72).strength(.23)).force('charge',d3.forceManyBody().strength(d=>d.kind==='asset'?-25:-125)).force('center',d3.forceCenter(w()/2,h()/2)).force('collision',d3.forceCollide().radius(d=>radius(d)+5));
const link=g.append('g').selectAll('line').data(data.edges).join('line').attr('class',d=>`link ${d.type}${d.in_cycle?' cycle':''}`).attr('stroke',d=>relColors[d.type]||'#64748b'),node=g.append('g').selectAll('g').data(data.nodes).join('g').attr('class','node').call(d3.drag().on('start',(e,d)=>{if(!e.active)sim.alphaTarget(.2).restart();d.fx=d.x;d.fy=d.y}).on('drag',(e,d)=>{d.fx=e.x;d.fy=e.y}).on('end',(e,d)=>{if(!e.active)sim.alphaTarget(0);d.fx=null;d.fy=null}));node.append('circle').attr('r',radius).attr('fill',d=>color(d.folder)).attr('class',d=>(impact[d.label]?.diagnostics||[]).some(x=>x.severity==='error')?'error':(d.risk_score/maxR>.58?'risky':''));node.append('text').attr('dx',d=>radius(d)+4).attr('dy',3).text(d=>d.label.split('/').pop());
const tip=document.querySelector('#tip');node.on('mouseenter',(e,d)=>{tip.innerHTML=`<b>${esc(d.label)}</b><br>${d.degree} relaciones · ${d.loc} líneas · riesgo ${d.risk_score}${d.purpose?'<br>'+esc(d.purpose):''}`;tip.style.opacity=1}).on('mousemove',e=>{tip.style.left=e.clientX+14+'px';tip.style.top=e.clientY+14+'px'}).on('mouseleave',()=>tip.style.opacity=0).on('click',(e,d)=>{e.stopPropagation();focus(d);details(d)});
function focus(d){const near=new Set([d.id]);data.edges.forEach(e=>{let s=edgeId(e.source),t=edgeId(e.target);if(s===d.id)near.add(t);if(t===d.id)near.add(s)});node.classed('dim',n=>!near.has(n.id));link.classed('dim',e=>edgeId(e.source)!==d.id&&edgeId(e.target)!==d.id)}function listing(title,items,cls=''){return items?.length?`<div class="section"><span class="label">${title}</span><div class="list">${items.slice(0,30).map(x=>`<div class="item ${cls}">${esc(x)}</div>`).join('')}</div></div>`:''}
function details(d){empty.classList.add('hidden');const box=document.querySelector('#details'),i=impact[d.label]||{},ds=i.diagnostics||[];box.classList.remove('hidden');box.innerHTML=`<div class="path">${esc(d.label)}</div><h2 class="title">${esc(d.label.split('/').pop())}</h2><div class="pills"><span class="pill">${esc(d.ext)}</span><span class="pill">${esc(d.folder)}</span>${d.in_cycle?'<span class="pill">En ciclo</span>':''}</div>${d.purpose?`<p class="purpose">${esc(d.purpose)}</p>`:''}<div class="metrics section"><div class="metric"><b>${d.degree}</b><span>Relaciones</span></div><div class="metric"><b>${d.risk_score}</b><span>Riesgo</span></div><div class="metric"><b>${d.loc}</b><span>Líneas</span></div><div class="metric"><b>${d.symbols?.length||0}</b><span>Símbolos</span></div></div>${listing('Colecciones',i.firestore_collections)}${listing('Consumido por',i.used_by)}${listing('Depende de',i.depends_on)}${listing('Exports',d.exports)}${listing('Diagnósticos',ds.map(x=>x.severity.toUpperCase()+' · '+x.message),ds.some(x=>x.severity==='error')?'error':'warn')}`}
function resetAll(){node.classed('dim',false).classed('hidden',false);link.classed('dim',false).classed('hidden',false);q.value='';folder.value='';relation.value=''}function filters(){let s=q.value.toLowerCase(),f=folder.value,r=relation.value,visible=new Set(data.nodes.filter(d=>(!f||d.folder===f)&&(!s||d.label.toLowerCase().includes(s)||(d.exports||[]).some(x=>x.toLowerCase().includes(s))||(d.symbols||[]).some(x=>x.name.toLowerCase().includes(s)))).map(d=>d.id));node.classed('hidden',d=>!visible.has(d.id));link.classed('hidden',e=>!visible.has(edgeId(e.source))||!visible.has(edgeId(e.target))||(r&&e.type!==r))}function fitAll(){let b=g.node().getBBox();if(!b.width)return;let scale=Math.min(.95,Math.min(w()/b.width,h()/b.height)*.82),x=w()/2-scale*(b.x+b.width/2),y=h()/2-scale*(b.y+b.height/2);svg.transition().duration(550).call(zoom.transform,d3.zoomIdentity.translate(x,y).scale(scale))}
[q,folder,relation].forEach(x=>x.addEventListener(x===q?'input':'change',filters));reset.onclick=resetAll;fit.onclick=fitAll;svg.on('dblclick.zoom',null).on('dblclick',resetAll);sim.on('tick',()=>{link.attr('x1',d=>d.source.x).attr('y1',d=>d.source.y).attr('x2',d=>d.target.x).attr('y2',d=>d.target.y);node.attr('transform',d=>`translate(${d.x},${d.y})`)});setTimeout(fitAll,1400);addEventListener('resize',()=>sim.force('center',d3.forceCenter(w()/2,h()/2)).alpha(.2).restart());
</script></body></html>"""
